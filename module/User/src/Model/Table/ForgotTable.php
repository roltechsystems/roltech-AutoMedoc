...
class ForgotTable extends AbstractTableGateway
{
	...
	public function deleteToken(string $userId)
	{
		$sqlQuery = $this->sql->delete()->where(['username' => $userId]);
		$sqlStmt  = $this->sql->prepareStatementForSqlObject($sqlQuery);

		return $sqlStmt->execute();
	}

	public function fetchToken(string $token, string $userId)
	{
		$sqlQuery = $this->sql->select()->where(['token' => $token])->where(['username' => $userId]);
		$sqlStmt  = $this->sql->prepareStatementForSqlObject($sqlQuery);
	...
	# saving the token
	public function saveToken(string $token, string $userId)
	{
		$values = [
			'username' => $userId,
			'token'   => $token,
